// Direct fish import using existing schema (without class column)
...
async function importFishData() {
  console.log('🐟 Starting fish import (simple schema)...');
  ...
  try {
    ...
    // Step 2: Insert fish data
    console.log('2️⃣ Importing fish species...');
    ...
    const { data: allFish, error: verifyError } = await supabase
      .from('fish_species')
      .select('id, common_name, family, invasive')
      .order('common_name');
    ...
    console.log(`   4. The Fresh/Salt filters will show "All" for now (until class column is added)`);
